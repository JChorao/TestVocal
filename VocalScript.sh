...
docker push $DOCKER_USERNAME/vocalscript-function:latest

docker build -t $DOCKER_USERNAME/vocalscript-app:latest .
docker push $DOCKER_USERNAME/vocalscript-app:latest
...
docker run -d \
  ...
  -p 5001:80 \  # Maps host 5001 to container's 80
